...
from ..setup import net_name, curve_generator, privatekey_class, publickey_class
...
class ExtendedKey(HexSerializable, metaclass=ABCMeta):
    ...
    def decode(string, check_network=True):
        ...
        if index >= ExtendedKey.first_hardened_index:
            index -= ExtendedKey.first_hardened_index
            hardened = True
        else:
            hardened = False
...
class ExtendedPrivateKey(ExtendedKey):
    ...
    @staticmethod
    def decode_key(keydata):
        return privatekey_class()(keydata[1:])

    def __init__(self, key, chaincode, depth, pfing, index, hardened=False):
        if not isinstance(key, privatekey_class()):
    ...
    def get_child(self, index, hardened=False):
        ...
        return ExtendedPrivateKey(privatekey_class()(k.to_bytes(32, 'big')),
                                  right,
                                  self.depth + 1,
...
class ExtendedPublicKey(ExtendedKey):
    ...
    def decode_key(self, keydata):
        return publickey_class()(keydata)

    def __init__(self, key, chaincode, depth, pfing, index, hardened=False):
        if not isinstance(key, publickey_class()):
    ...
    def get_child(self, index, hardened=False):
        ...
        return ExtendedPublicKey(publickey_class().from_point(point), right, self.depth + 1, self.get_fingerprint(), index,
                                 False)
